// ============================================================
// ConfigFileWriter.cs —— 把运行期调整写回**当前档案**
//
// 【难点不在"写"，在"别写坏别人的东西"】
//   （2026-09-20 多档案之后：这里收到的是 `ProfileLocator` 算出来的路径，
//     `profiles\<当前档案名>.json` —— 换档案之后窗口位置要存进新的那一份。
//     以前调用方自己拼 `Path.Combine(exeDir, "config.json")`，
//     于是"读哪份"和"写哪份"是两份独立的真相。）
//
//   档案文件里还有用户手写的一堆内容：中文注释键（`_说明` 那种）、模型路径、
