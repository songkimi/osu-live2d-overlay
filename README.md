...
6. 开一局游戏：连击、断连击、语音都会跟着动。**打完进结算时，角色会按这一局的准确率给一个反应。**

**想让角色对"点一下"有反应**：在「联动与窗口 → 悬浮窗」里给对应界面打开**「允许触摸」**，
反应配在「行为 → 被摸到的时候」（点一下会从那里随机挑一条播）。
...
| **行为** | 什么时候做什么：常态区间 / 触发点 / 失误反应 / 触摸反应 / 结算反应 / 常驻组件 + 表现策略 + 语音与口型。**左边是模型里的表情与动作清单，点一下右边就试播** |
| **联动与窗口** | 悬浮窗的全局行为（吸附距离、位置锁定、全局热键）+ 四个界面各自的窗口位置、尺寸、角色站位、是否允许触摸 |
...
| 首次启动游戏，快速来到选歌界面发现悬浮窗几秒后才跑到对应位置 | **不是 bug**：那几秒里 osu 还没把新界面报出来，程序在等它。状态一到，窗口和角色在 **10 毫秒内**就位（日志里「界面切换」和「窗口：位置」会挨着出现）。启动时先开 osu、进到主菜单，能少遇到,这是刚进游戏才会出现的问题|
| 连击到了不换表情 | 标识和模型对不上 —— 去「行为」页左边点一下引用的表情查看 |
| 打完一局结算了，角色却没什么反应 | 「行为 → 结算反应」里的档位之间**可能有空隙**。区间是 `[最小, 最大)`，**落不进任何一档就不播**。让相邻两档首尾相接（前一档的"最大" = 后一档的"最小"），**最后一档的"最大"留空 = 以上**（这样满分 100 也有人接） |
| 没有声音 | 「行为 → 语音与口型」里的目录与情绪文件夹是否配对了 |
| 我确信这就是BUG | 「数据与档案 → 日志与诊断」里可以导出日志，直接拿给agent分析，或说明如何复现并告知开发者(记得带上诊断包) |
